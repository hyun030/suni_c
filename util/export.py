...
def create_enhanced_pdf_report(
    ...
    insights: str = None,
    selected_charts: list = None,  # plotly Figure 리스트
    ...
    """
    PDF 리포트 생성 함수
    - financial_data, news_data, quarterly_df는 DataFrame으로,
      필요 없는 컬럼은 미리 제거해서 넘기세요.
    - selected_charts는 plotly Figure 리스트로 차트 이미지를 넣습니다.
    - 폰트 경로는 반드시 본인 PC의 실제 위치로 변경하세요.
    """

    # 1. 폰트 경로 설정 (본인 PC 맞게 반드시 수정 필요)
    font_paths = {
...
def create_excel_report(financial_data: pd.DataFrame) -> bytes:
    """
    Excel 리포트 생성 함수
    - financial_data는 DataFrame (필수)
    - 필요 없는 컬럼은 미리 제거 후 넘기세요
    """
