# 3. 뉴스 하이라이트 및 종합 분석
    if news_data is not None and hasattr(news_data, "empty") and not news_data.empty:
        ...
        if insights:
            ...
            def ascii_to_table(lines):
                ...
                tbl.setStyle(TableStyle([
                    ...
                    ('FONTSIZE', (0,0), (-1,-1), 8),
                    ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.whitesmoke, colors.HexColor('#F0F8FF')]),
                ]))
                return tbl
        ...
        # 뉴스 데이터가 없어도 AI 인사이# -*- coding: utf-8 -*-
import io
import os
import tempfile
import pandas as pd
...
def create_enhanced_pdf_report(
    ...
    if financial_data is not None and hasattr(financial_data, "empty") and not financial_data.empty:
        ...
        max_rows_per_table = 25  # 한 페이지에 들어갈 수 있는 최대 행 수
        ...
        if total_rows <= max_rows_per_table:
            ...
            for i in range(0, total_rows, max_rows_per_table):
                ...
                if end_idx < total_rows:
                    story.append(PageBreak())  # 다음 테이블을 위한 페이지 나누기
        
        story.append(Spacer(1, 18))

    # Plotly 차트 PNG 변환 헬퍼 함수
    ...
    if news_data is not None and hasattr(news_data, "empty") and not news_data.empty:
        ...
        if insights:
            ...
            def clean_ai_text_for_news(raw: str):
                """AI 인사이트 텍스트 정리 (뉴스 섹션용)"""
                raw = re.sub(r'[*_#>~]', '', raw)  # 마크다운 문자 제거
                blocks = []
                for line in raw.splitlines():
                    line = line.strip()
            ...
            def ascii_to_table_news(lines):
                """ASCII 표를 reportlab 테이블로 변환 (뉴스 섹션용)"""
                if not lines:
                    return None
            ...
            # AI 인사이트 텍스트 처리
            blocks = clean_ai_text_for_news(str(insights))
            ascii_buffer = []
            ...
            for typ, line in blocks:
                ...
                # 버퍼에 표 데이터가 있으면 테이블 생성
                if ascii_buffer:
                    tbl = ascii_to_table_news(ascii_buffer)
                    if tbl:
                        story.append(tbl)
                    story.append(Spacer(1, 12))
                    ascii_buffer.clear()
        ...
        if insights:
            ...
            def clean_ai_text_standalone(raw: str):
                """AI 인사이트 텍스트 정리 (독립 섹션용)"""
                raw = re.sub(r'[*_#>~]', '', raw)
                blocks = []
                for line in raw.splitlines():
                    line = line.strip()
            ...
            def ascii_to_table_standalone(lines):
                """ASCII 표를 reportlab 테이블로 변환 (독립 섹션용)"""
                if not lines:
                    return None
                ...
                tbl.setStyle(TableStyle([
                    ...
                    ('FONTNAME', (0,1), (-1,-1), registered_fonts.get('Korean', 'Helvetica')),
                    ('FONTSIZE', (0,0), (-1,-1), 8),
                    ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.whitesmoke, colors.HexColor('#F0FFF0')]),
                ]))
                return tbl

            # AI 인사이트 텍스트 처리
            blocks = clean_ai_text_standalone(str(insights))
            ascii_buffer = []
            ...
            for typ, line in blocks:
                ...
                # 버퍼에 표 데이터가 있으면 테이블 생성
                if ascii_buffer:
                    tbl = ascii_to_table_standalone(ascii_buffer)
                    if tbl:
                        story.append(tbl)
                    story.append(Spacer(1, 12))
                    ascii_buffer.clear()
